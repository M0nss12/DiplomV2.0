...
// Определение 30 категорий и логика различающихся характеристик
const catsInfo = [
...
async function seed() {
    try {
        console.log('🧹 Очистка...');
        const tables = ['wishlists', 'reviews', 'order_items', 'orders', 'product_stocks', 'warehouses', 'products', 'categories', 'brands', 'users'];
        ...
        const { data: dbUsers } = await supabase.from('users').insert(Array.from({ length: 30 }).map((_, i) => ({
            role: i === 0 ? 'admin' : 'user',
            email: `user${i+1}@apexdrive.ru`, phone_number: `+790088800${i.toString().padStart(2, '0')}`,
            password_hash: '123', first_name: 'Имя' + i, last_name: 'Фамилия' + i,
            avatar_url: `${STORAGE_URL}/avatars/${(i % 6) + 1}.png`, saved_address: cities[i % cities.length]
        }))).select();
        ...
        for (let i = 0; i < 30; i++) {
            ...
            for (const sc of scenarios) {
                ...
                const { data: ord } = await supabase.from('orders').insert({
                    user_id: user.id, payment_method: sc.pm, payment_status: sc.ps, delivery_status: sc.ds,
                    total_price: prod.price, delivery_address: `${user.saved_address}, ПВЗ Тестовый`
                }).select().single();
                await supabase.from('order_items').insert({ order_id: ord.id, product_id: prod.id, quantity: 1, unit_price: prod.price });
                if (sc.ds === 'delivered') {
                    reviewsData.push({ product_id: prod.id, user_id: user.id, rating: 5, comment: 'Отличный товар, характеристики как в описании!', is_approved: true });
                    reviewsData.push({ product_id: prod.id, user_id: dbUsers[(i+1)%30].id, rating: 4, comment: 'Хорошее качество, рекомендую.', is_approved: true });
                }
            }
        }
        ...
        console.log('✅ База успешно заполнена! 30 категорий, 60 товаров, 200 остатков, 90 заказов.');
